...
## 既存チェック布を活かしたレシピ（2026-09-06・最新）

レシピの大きなピンクの囲み枠を外し、背景モデルの右側 `GinghamTowel` をタルトの下敷きとして使用。見出しの素材は縫い目のある生成り色の `HudRecipeLabel.png` に変更しました。布モデルは形状とチェック模様を保った等倍拡縮で、ステージや画面比率が変わっても共有設計領域へ合わせます。日本語は従来どおり実テキストです。

画面確認: `Assets/Docs/UI/hud-recipe-cloth.png`、`hud-recipe-cloth-4x3.png`、`hud-recipe-cloth-stage2.png`。生成方法と最終プロンプトは `Docs/UI/art-prompts.md`。
