...
- 見下ろし画面で顔を見せるため、モデルを画面奥へ15度傾けます。移動方向にかかわらずカメラの奥側を使い、角度は `Visual > FruitModelVisual > View Tilt Degrees` で変更できます。仰向けで回転する滑走中だけ傾きを外し、垂直なY軸回転を維持します。モデルの親に実行時生成する `View tilt` で処理し、アニメーションや移動判定は変更しません。
