...
| クッキー `C` | 滑走衝突3回で破壊。最後の一撃は通り抜け、5秒後に復帰 |
| 移動シュレッダー `H` / `V` | 横/縦に1マス/秒で往復。障害物で反転し、滑走中のフルーツを収穫 |
| スコーン `1`〜`4` | 白い斜面で90度曲がる。裏側からは反転。4方向に配置可能 |
| フリーザー `F` | 歩行・滑走を3秒間0.35倍に減速。氷の殻で状態を表示 |

四角いビスケット壁 `#` は壊れません。丸いクッキーが破壊対象です。壊れたクッキーは復帰先にフルーツや移動刃がいる間は待機し、戻ると耐久を全回復して、そのマスのよだれを消します。フリーザーは踏み直すと時間を更新し、減速は重複しません。すべてのギミックの時間は一時停止とヒットストップに連動します。

ゼリー・スコーン・クッキーは通常歩行では避け、滑走中に作用します。移動刃は歩行中のフルーツを避けて反転し、通常の歩行では収穫されません。移動経路の途中で滑るフルーツと接触した場合も判定し、メロンは従来どおり2回の有効な衝突が必要です。ギミックによる反射・破壊通過・減速では連鎖数をリセットせず、ほかのフルーツを巻き込んだときに連鎖を増やします。
...
- ステージ名、サイズ、必要フルーツ数、脱出上限、出現間隔、よだれの寿命、クッキーの耐久・復帰時間、刃の速度、凍結時間・速度倍率を編集可能。
