...
| スコーン `1`〜`4` | 歩行・滑走とも上に乗ってから、白い斜面で90度、裏側からは反転。滑走20回目で壊して直進し、一時的に床扱い。5秒後に復帰 |
...
ゼリーと壊れていないクッキーは通常歩行では避けますが、オレンジは歩行中にもクッキーを削れます。スコーンには全フルーツが歩いて乗れ、マスの中心に到達してから斜面の向きに従って進路を変えます。歩行では耐久を削らず、壁で曲がる左右の順番も進めません。滑走中の反射・破壊は全フルーツ共通です。移動刃は歩行中のフルーツを避けて反転し、通常の歩行では収穫されません。移動経路の途中で滑るフルーツと接触した場合も判定し、メロンは従来どおり2回の有効な衝突が必要です。ギミックによる反射・破壊通過・減速では連鎖数をリセットせず、ほかのフルーツを巻き込んだときに連鎖を増やします。
