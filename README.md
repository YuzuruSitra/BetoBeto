...
よだれは初期設定で10秒で消え、再使用待ちは0.3秒です。驚かすには再使用待ちがなく、単押しの連続使用や発動直後の再チャージが可能です。壁・ゼリー・壊れていないクッキーやスコーン・パイプ・シュレッダーにはよだれを置けません。外周の床や壊れたお菓子のマスには置けます。氷ブロックの生成は廃止しました。
...
よだれに触れたフルーツは、そのときの進行方向へ滑り始め、ギミックで反転・転向しながら進みます。ほかのフルーツに触れると巻き込み、同じ連鎖の基準速度が上がります。凍結中は各フルーツに減速倍率を掛けます。得点は1体につき100点×連鎖数。同じ刃での連続収穫は得点表示を合算します。壊れない壁への激突ではフルーツが潰れて跳ね返り、短く硬直します。壁への激突だけでは収穫されません。
...
よだれで滑り始めたフルーツを、反転・転向・破壊・減速でつないで刃へ送ります。両ステージに5種類を配置し、内側の固定壁を減らして経路を読みやすくしています。
...
| フリーザー `F` | 歩行・滑走を3秒間0.35倍に減速。氷の殻で状態を表示 |

四角いビスケット壁 `#` は壊れません。クッキーは破壊後20秒、スコーンは5秒で復帰します。どちらも復帰先にフルーツや移動刃がいる間は待機し、戻ると耐久を全回復して、そのマスのよだれを消します。復帰時間はステージごとに調整できます。フリーザーは踏み直すと時間を更新し、減速は重複しません。すべてのギミックの時間は一時停止とヒットストップに連動します。
...
1番目のステージは、中央のゼリーと周囲のスコーンで方向を変え、クッキーに囲まれた固定刃や下段の横移動する刃へ送る配置です。2番目のステージには、右上のスコーンで下へ、右下のスコーンで左へ曲げて、クッキーの先の刃へ送る経路があります。フリーザーと動く刃を組み合わせ、凍結による到着時間の変化も利用できます。

数値はステージエディタの「ギミック」で調整できます。実装は `Stage/GimmickRules.cs`（反射・耐久・往復移動）、`Stage/GimmickController.cs`（ゲーム進行との連動・破壊表示・移動接触）、`Enemies/FruitAgent.cs`（滑走・凍結・連鎖）です。5種類の見た目は `Prefabs/Stage` のPrefabとして差し替えられます。
...
- 5種類のギミックを追加配置可能。移動刃は横・縦のブラシを選び、スコーンはRまたは回転ボタンで90度ずつ回転。
- Ctrl+Z：Undo。Ctrl+Shift+Z / Ctrl+Y：Redo。
- ステージ名、サイズ、必要フルーツ数、脱出上限、出現間隔、よだれの寿命、クッキーの耐久、クッキー・スコーンそれぞれの復帰時間、刃の速度、凍結時間・速度倍率を編集可能。
